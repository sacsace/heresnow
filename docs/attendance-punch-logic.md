...
| `AUTO_CHECKOUT_CUTOFF_HOUR` | **06:00** | 출근일 다음날(회사 TZ)까지 미퇴근 시 서버 자동 퇴근 |

### 서버 자동 퇴근 (`lib/autoCheckOut.ts`)

- **조건:** 마지막 기록이 `CHECK_IN` 이고, **출근일(회사 타임존) 다음날 06:00** 을 지난 뒤 (출퇴근·상태 API 등에서 `applyPendingAutoCheckOut*` 실행 시)
- **기록 시각:** 실시간이 아니라 **출근일 정규 퇴근 시각** (`resolveAutoCheckOutTimestamp` — 근무표·`workEndTime` 등)
- **표시:** memo `[AUTO_CHECKOUT]`, UI `(자동 퇴근)`
...
| 케이스 | 동작 |
|--------|------|
| **출근일 다음날 06:00(회사 TZ) 미퇴근** | 서버가 정규 퇴근 시각으로 `CHECK_OUT` 자동 생성 |
...
| 상황 | 동작 | 기록 시각 |
|------|------|-----------|
| 월 09:00 출근, 퇴근 없음 → **화 06:00** 이후 API 조회 | 서버 자동 `CHECK_OUT` | 월 정규 퇴근(예: 18:00) |

**직원이 직접 늦게 퇴근 버튼을 누를 때:**
