...
async function main() {
  ...
  if (!existing) {
    if (!isStrongPassword(password)) {
      console.warn(
        "[bootstrap] 경고: SEED_SUPER_ADMIN_PASSWORD 가 비밀번호 정책(6자 이상)을 충족하지 않습니다. 기존 운영 호환을 위해 생성은 진행합니다."
      );
    }
  ...
  if (rotate) {
    if (!isStrongPassword(password)) {
      console.warn(
        "[bootstrap] 경고: SEED_SUPER_ADMIN_ROTATE_PASSWORD=1 이지만 비밀번호가 정책(6자 이상)을 충족하지 않아 비밀번호 갱신을 건너뜁니다."
      );
    } else {
