...
export default function SeatUnavailablePage() {
  ...
  return (
    <div className="mx-auto max-w-lg py-10">
      <h1 className={pageTitle}>{t("employee.seatUnavailableTitle")}</h1>
      <p className={pageSubtitle}>{t("employee.seatUnavailableLead")}</p>
      <div className={`${card} mt-8`}>
        <div className={`${cardBody} space-y-3 text-[0.9375rem]`}>
          <p>{t("employee.seatUnavailableHint")}</p>
          <p className="text-[var(--apple-label-secondary)]">
            {t("employee.seatUnavailableAdminNote")}
          </p>
        </div>
      </div>
